...
if __name__ == '__main__':
    args = {
        'MODEL_NAME': './Models/rwkv/RWKV-x070-World-0.1B-v2.8-20241210-ctx4096', #模型文件的名字，pth结尾的权重文件。
    ...
    print(f"tokenizer.encode(prompt):{len(tokenizer.encode(prompt))} {tokenizer.encode(prompt)}")
    # 编码初始字符串
    token = torch.tensor(tokenizer.encode(prompt), dtype=torch.int64, device=device).expand([BATCH_SIZE, -1])
