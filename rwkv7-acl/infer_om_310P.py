...
########################################################################################################
def sample_single_value(sorted_probs: torch.Tensor) -> int:
    """
    从单个已排序的softmax概率分布中采样一个值
    
    参数:
        sorted_probs: 单个已排序的概率分布张量，形状为 (n_categories,)
    
    返回:
        采样的索引（整数）
    """
    # 生成随机数
    u = torch.rand(1, device=sorted_probs.device).item()
...
def sample_logits(logits, temperature:float=1.0, top_p:float=1.0, top_k:int=0):
    ...
    '''
    print(temperature,top_p,top_k)
    if top_k > 0:
        probs = sorted_probs[0:top_k]
    ...
    '''    
    return c[0].item()
    #return sample_single_value(probs)
...
t1 = time.time()
np.array(output[0])
init_out = torch.from_numpy(np.array(output[0])).view(-1).npu()
...
for TRIAL in range(NUM_TRIALS):
    ...
    for i in range(LENGTH_PER_TRIAL):
        ...
        output = session.infer(input,out_array=False)
        output[0].to_host()
        out = torch.from_numpy(np.array(output[0])).view(-1).npu()
        state = output[1:]
