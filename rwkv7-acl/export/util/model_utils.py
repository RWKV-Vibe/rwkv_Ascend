...
def get_dummy_state(batch_size, model_cfg, device):
    def _cache(shape, fp16):
        if fp16:
            return torch.zeros(shape, dtype=torch.float16).to(device=device).npu()
        else:
            return torch.zeros(shape).to(device=device).npu()
...
def get_dummy_input_for_rwkv_causal_llm(batch_size, input_length, device, model_cfg=None):
    input_ids = torch.LongTensor([[0]*input_length for _ in range(batch_size)]).to(device).npu()
