    ...
    __aicore__ inline void Process()
    {
        ...
        for (uint32_t h = 0; h < this->headPerCore; h++)
        {
            ...
            for (uint32_t tile = 0; tile < this->tileNum; tile++)
            {
                ...
                Compute(kLocal, vLocal, wLocal, rLocal, abLocal, oLocal, stateLocal, broadLocal0, broadLocal1, broadLocal2, h, 
...
private:
    ...
    __aicore__ inline void Compute(LocalTensor<float> kLocal, LocalTensor<float> vLocal, LocalTensor<float> wLocal, LocalTensor<float> rLocal, 
                                   LocalTensor<float> abLocal, LocalTensor<float> oLocal, LocalTensor<float> stateLocal,
                                   ...
                                   uint32_t currentTileLength)
    {
        ...
        // DumpTensor(stateLocal[offset0], 0, 4096); 
        ...
        for (uint32_t t = 0; t < currentTileLength; t++)
        {
            ...
            // broadcast a from [1, N] to [N, N]
            BroadCast<float, 2, 0>(broadLocal0, abLocal[t * this->HEAD_SIZE], broadDstShape, broadSrcShape);
            PipeBarrier<PIPE_V>();
            // DumpTensor(broadLocal0, 0, this->HEAD_ELEMENTS);
            // DumpTensor(stateLocal[offset0], 1, this->HEAD_SIZE);
            Mul(stateLocal[offset2], broadLocal0, stateLocal[offset0], this->HEAD_ELEMENTS);
            PipeBarrier<PIPE_V>();
            // DumpTensor(stateLocal[offset2], 2, this->HEAD_ELEMENTS);

            //DumpTensor(stateLocal[offset2], 2, this->HEAD_SIZE);

            // compute sa = reduceSum(astate), shape: N, broadLocal0
            // mask=N, repeatTimes=N, dstRepStride=1, srcBlkStride=1, srcRepStride=N*sizeof(float)/32=4
            WholeReduceSum(broadLocal0, stateLocal[offset2], this->HEAD_SIZE,
                                this->HEAD_SIZE, 1, 1, this->HEAD_SIZE * sizeof(float) / 32);
            
            // DumpTensor(broadLocal0, 3, this->HEAD_SIZE);
            // compute sab = sa * b, shape:N * N, broadLocal0
            // broadcast b from [1, N] to [N, N], broadcast sa from [N, 1] to [N, N]
            BroadCast<float, 2, 0>(broadLocal1, abLocal[this->tileLength * this->HEAD_SIZE + t * this->HEAD_SIZE], broadDstShape, broadSrcShape);
            PipeBarrier<PIPE_V>();
            // DumpTensor(broadLocal1, 4, this->HEAD_ELEMENTS);
            BroadCast<float, 2, 1>(broadLocal2, broadLocal0, broadDstShape, broadSrcShape);
            PipeBarrier<PIPE_V>();
            // DumpTensor(broadLocal2, 5, this->HEAD_ELEMENTS);
            Mul(broadLocal0, broadLocal2, broadLocal1, this->HEAD_ELEMENTS);
            PipeBarrier<PIPE_V>();
            // DumpTensor(broadLocal0, 6, this->HEAD_ELEMENTS);
            ...
            PipeBarrier<PIPE_V>();
            // DumpTensor(stateLocal[offset1], 3, this->HEAD_ELEMENTS);
            
            // compute state =  state * exp(-exp(w)), shape:N * N, state
            ...
            PipeBarrier<PIPE_V>();
            Mul(stateLocal[offset0], stateLocal[offset0], broadLocal1, this->HEAD_ELEMENTS);
            PipeBarrier<PIPE_V>();
            // DumpTensor(stateLocal[offset0], 7, this->HEAD_ELEMENTS);

            // compute state = state * w + kv + sab, shape:N * N, offset0
            Add(broadLocal2, stateLocal[offset0], stateLocal[offset1], this->HEAD_ELEMENTS);
            PipeBarrier<PIPE_V>();
            // DumpTensor(broadLocal2, 8, this->HEAD_ELEMENTS);
            Add(stateLocal[offset0], broadLocal2, broadLocal0, this->HEAD_ELEMENTS);
            PipeBarrier<PIPE_V>();
            // DumpTensor(stateLocal[offset0], 9, this->HEAD_ELEMENTS);

            // compute out = state * r, shape:N * N, offset2
            // broadcast r from [1, N] to [N, N]
            BroadCast<float, 2, 0>(broadLocal0, rLocal[t * this->HEAD_SIZE], broadDstShape, broadSrcShape);
            PipeBarrier<PIPE_V>();
            Mul(stateLocal[offset2], broadLocal0, stateLocal[offset0], this->HEAD_ELEMENTS);
            PipeBarrier<PIPE_V>();
            // DumpTensor(stateLocal[offset2], 4, this->HEAD_ELEMENTS);
            ...
            // if (t % 2 == 1)
            // {
                // DumpTensor(stateLocal[offset0], 4, this->HEAD_ELEMENTS);
