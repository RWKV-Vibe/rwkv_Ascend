...
def _build_args(k_in__, v_in__, w_in__, r_in__, u_in__, hi_in__, o_out_, ho_out_):
    __inputs__ = []
    for arg in [k_in__, v_in__, w_in__, r_in__, u_in__, hi_in__]:
        if arg != None:
...
@tbe_register.register_operator("wkv6", trans_bool_to_s8=False)
...
def wkv6(k_in__, v_in__, w_in__, r_in__, u_in__, hi_in__, o_out_, ho_out_, kernel_name="wkv6", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(k_in__, v_in__, w_in__, r_in__, u_in__, hi_in__, o_out_, ho_out_)
    ...
    origin_func_name = "wkv6"
    ascendc_src_dir_ex = "wkv6"
    ascendc_src_dir = "wkv6"
    ascendc_src_file = "wkv6.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator wkv6, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "wkv6"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[k_in__, v_in__, w_in__, r_in__, u_in__, hi_in__], origin_outputs = [o_out_, ho_out_],\
...
def op_select_format(k_in__, v_in__, w_in__, r_in__, u_in__, hi_in__, o_out_, ho_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(k_in__, v_in__, w_in__, r_in__, u_in__, hi_in__, o_out_, ho_out_)
    result = check_op_cap("op_select_format", "wkv6", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(k_in__, v_in__, w_in__, r_in__, u_in__, hi_in__, o_out_, ho_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(k_in__, v_in__, w_in__, r_in__, u_in__, hi_in__, o_out_, ho_out_)
    result = check_op_cap("get_op_specific_info", "wkv6", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
